# CGI356Lib.py v3.0.0  2024-10-19
...
class Response:
  ...
  # Cheetah ファイルをレンダリングして応答として返す。path はそのパス名。
  def sendCheetah(self, path:str, charset="", cookie=True, headers=True, embed=None):
    buff = ""
    if headers and len(self.Headers) > 0:
      for h in self.Headers:
        buff += h + "\n"
    ...
    with open(path, mode="rt", encoding="utf-8") as f:
      cheetah = f.read()
      result = str(Cheetah(cheetah, searchList=[embed]))
      buff += result
      print(buff)
    return

  # Mako ファイルをレンダリングして応答として返す。path はそのパス名。
  def sendMako(self, path:str, charset="", cookie=True, headers=True, **embed):
    buff = ""
    if headers and len(self.Headers) > 0:
      for h in self.Headers:
        buff += h + "\n"
    ...
    result = str(template.render(**embed))
    buff += result
    print(buff)
    return
