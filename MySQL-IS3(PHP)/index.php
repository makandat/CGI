...
class MyPage extends WebPage {
  ...
  public function __construct(string $filePath = null) {
    ...
    $this->setPlaceHolder('content', $this->getContent());
    $this->setPlaceHolder('title', '(New) MySQL-IS2');
  }
  ...
  public function getContent() : string {
    $cc = 'databases';
    if ($this->isParam('view')) {
      $cc = $this->getParam('view');
    }
    ...
    switch ($cc) {
      ...
      case 'tables':
        // テーブル一覧
        if ($this->isParam('database')) {
          $content = $this->listTables($this->getParam('database'));
        }
        else {
      ...
      case 'views':
        // ビュー一覧
        if ($this->isParam('database')) {
          $content = $this->listViews($this->getParam('database'));
        }
        else {
      ...
      case 'indexes':
        // インデックス一覧
        if ($this->isParam('database')) {
          $content = $this->listIndexes($this->getParam('database'));
        }
        else {
      ...
      case 'routines':
        // ルーチン一覧
        if ($this->isParam('database')) {
          $content = $this->listRoutines($this->getParam('database'));
        }
        else {
      ...
      case 'triggers':
        // トリガ一覧
        if ($this->isParam('database')) {
          $content = $this->listTriggers($this->getParam('database'));
        }
        else {
      ...
      case 'columns':
        // テーブルのカラム定義
        $content = $this->listColumns($this->getParam('database'), $this->getParam('table'));
        break;

      case 'viewdef':
        // ビュー定義
        $content = $this->getViewDef($this->getParam('database'), $this->getParam('table'));
        break;

      case 'routinedef':
        // ルーチン定義
        $content = $this->getRoutineDef($this->getParam('database'), $this->getParam('routine'));
        break;

      case 'triggerdef':
        // トリガ定義
        $content = $this->getTriggerDef($this->getParam('database'), $this->getParam('trigger'));
        break;
  ...
  private function selectDatabase($view) {
    ...
    foreach ($dataTable as $row) {
      ...
      $content .= WebPage::anchor('index.php?view=' . $view . '&amp;database=' . $row['SCHEMA_NAME'], $row['SCHEMA_NAME']);
      $content .= '</li>';
    }
    $content .= '</ol></div>';
    return $content;
  ...
  private function listDatabases() {
     ...
     foreach ($dataTable as $row) {
       $buff .= '<tr>';
       $anchor = WebPage::anchor("index.php?view=tables&amp;database=" . $row['SCHEMA_NAME'], $row['SCHEMA_NAME']);
       $buff .= "<td>".$anchor."</td>";
  ...
  private function listTables($database) {
     ...
     foreach ($dataTable as $row) {
       $buff .= '<tr>';
       $buff .= '<td>'.WebPage::anchor('index.php?view=columns&amp;database=' . $database . '&amp;table=' . $row['TABLE_NAME'], $row['TABLE_NAME']).'</td>';
  ...
  private function listViews($database) {
     ...
     foreach ($dataTable as $row) {
       $buff .= '<tr>';
       $buff .= '<td>' . WebPage::anchor('index.php?view=viewdef&amp;database=' . $database . '&amp;table=' . $row['TABLE_NAME'], $row['TABLE_NAME']) .'</td>';
       $buff .= '</td>';
     }
  ...
  private function listRoutines($database) {
     ...
     foreach ($dataTable as $row) {
       $buff .= '<tr>';
       $buff .= '<td>' . WebPage::anchor('index.php?view=routinedef&amp;database=' . $database . '&amp;routine=' . $row['ROUTINE_NAME'], $row['ROUTINE_NAME']) .'</td>';
  ...
  private function listTriggers($database) {
     ...
     foreach ($dataTable as $row) {
       $buff .= '<tr>';
       $buff .= '<td>' . WebPage::anchor('index.php?view=triggerdef&amp;database=' . $database . '&amp;trigger=' . $row['TRIGGER_NAME'], $row['TRIGGER_NAME']) .'</td>';
...
//  メインプログラム
//  ==================
$p = new MyPage('templates/index.html');
$p->echo();
